...
object IOSpec extends Specification with HttpClient {
    ...
    "HttpServiceClient" should {
      ...
      "search" in {
        ...
        executeHttpPostIO("http://localhost:9000/search?q=cat", newUser).unsafeRunSync.body must beEqualTo("""{"searchString":"cat","results":[{"name":"Cat","desc":" A small, typically furry, carnivorous mammal. They are often called house cats when kept as..."},{"name":"Cat Stevens","desc":"A British singer-songwriter and multi-instrumentalist."},{"name":"Computed axial tomography","desc":"A CT scan, also known as computed tomography scan, makes use of computer-processed combinations..."}]}""")
      }
    //   "search_terms GET" in {
    //     UserSearchRepository.clear
    //     val newUser = Map("username"-> "user1", "password" -> "pass1")
